...
function defaultContentString(node) {
    ...
    // pause(node)
    const transform = (c, i, a) => {
        // console.log([c.text, c.newlines])
        const ref = {
            0: '',
            1: '\n',
            2: '\n#v(1em)\n',
        }
        const text = typstSentenceTransform(c.text)
        const newlines = ref[c.newlines]
        let v = text + newlines
        const next = a[i + 1]
        if (c.newlines == 0 && next) {
            v += '\\'
        }
        // console.log({v})
        return v
    }
    let s = node.contents.map(transform).join('\n')
    // throw {s}
    if (node.attributes) {
        return attrWrap('text', node.attributes, s)
    }
    return s
}
function wrapContent(content) {
    return wrap(newlineIndent(content), '[]')
}
/* @bookmark 1708816988 typst */
/* @bookmark 1708836797 com */
...
const typstBlocks = [
...

    {
        ...
        match: /^-{3,}$/,
        run() {
            const length = this.matches[0].length
            if (length == 3) {
                this.getBlock()
                this.token.set("type", "frontMatter")
            } else {
                this.token.set("type", "dialogueDelimiter")
            }
        }
    },

    {
        priority: "",
        type: "function",
        match: /^(?:(?:async )?function (\w+)\(|(\w+)\().*?{$/,
        run() {
            const name = this.matches[0] || this.matches[1]
            this.token.set("functionName", name)
    ...
    },
    {
        ...
        run() {
            ...
            // this.eat()
            if (this.currentInd < this.startInd) {
                this.indentTracker.state.remove(this.block)
                return false
            }
        },
    },
    {
        ...
        visit(node) {
            return node.computedText + newlineIndent(join(node.children.map(this.visit)))
        }
...
// /home/kdog3682/2024-javascript/txflow/tests/typst.a.js
        ...
        visit(node) {
            // we need the depth
            // the depth will tell u whether or not to use smth

            /* @bookmark 1708836797 com */
            const isComp = node.parent.type == 'component'
            // console.log(node.attributes, node.properties, node.name)
            const prefix = isComp ? '' : '#'
            ...
            let s = ''
                ...
                let name = ref[node.name] || node.name
            let t = prefix + name + "("
            // s += t + node.name
            // s += '('
                if (node.properties) {
                    if (node.attributes) {
                        Object.assign(node.attributes, node.properties) 
                    } else {
                        node.attributes = node.properties
                    }
                }
            if (node.attributes) {
...

    {
        ...
        visit(node) {
            ...
            return isComp ? text.replace('#', '') : text
        }
    },
    {
        priority: "A",
        ...
        visit(node) {
            console.log(node.prevSibling.attributes)
            console.log(node.attributes)
            console.log(node.computedText)
            pause('a')
            // pause('a')
        }
    },
...

    {
        ...
        match: /^\$\$\$/,
        run() {
            this.getBlock({ includeEndpoint: false, includeStartpoint: false })
        },
        visit(node) {
            // console.log((node.contents)); throw "throwing"
            const computedText = node.contents.filter((x) => x.text).map((x) => x.text).join(' \\\n')
            // console.log(computedText); throw "computedText"
            return indentWrap(computedText, '$')
        }
    },
    ...
    // },
    {
        ...
        visit(node) {
            ...
            // pause('hi')
            const isComp = node.parent.type == 'component'
            const content = defaultContentString(node)
            return isComp && !node.attributes ? wrapContent(content) : content
        }
    },
    {
        type: 'Root',
...

    {
        ...
        visit(node) {
            return node.children.map(this.visit).join('\n')
            return wrapContent(node.children.map(this.visit).join('\n'))
        }
    },
]
function attrWrap(fnKey, attributes, s) {
    // return `${fnKey}()`
    const attrString = toStringArgumentPretty(attributes, {lang: "typst", max_length: 1000, noWrapper: true})
	return `${fnKey}(${newlineIndent(attrString)}, "${s}")`
}
