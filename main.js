...
function factory(flavor, topOptions, innerOptionsFromTop) {
    ...
    // the visitor is created here
    // it is here that we decide whether to visit of json or str
    // mode: 'json' | 'str' | ''
    const visitor = createVisitor(options.blocks, visitOptions)

    return function txflow(s, innerOptions) {
        const tokens = scanner.scan(s)
        const root = parser(tokens)
        if (denoDebug("txflow_ast")) {
            console.log(root)
            return
            // return root
        }
        ...
        if (options.state) {
            ...
            // this visitor.state sometimes represents ComponentState
            // typst does not use it however
            // which is why only "sometimes"
