...
import {typstBlocks} from "./simpleTypstBlocks.js"
import {vueBlocks} from "./vueBlocks.js"
import {vuemdBlocks} from "./vuemdBlocks.js"
function getBlocks(flavor) {
    ...
    const ls_matchf = (x)=> {
        ...
        if (isArray(x)) {
            const r = reTemplate(/^($1)\b/, x)
            return (o) => match(o.text, r)
        }
    ...
    const final = base.filter(filter).map(mapper)
    return final
    const sorter = (x) => priorityValue(x.priority)
    return sort(final, sorter)
}
