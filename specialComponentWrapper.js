...
const containerStyleRef = {
    ...
    columns({size, gap, left}) {
        return 
        const gridTemplateColumns = `repeat(${size}, minmax(${left}px, 1fr))`
        return {
            display: 'grid',
            gap: `${gap}px`,
...
function specialComponentWrapper(node) {
    ...
    if (component == 'container') {
        ...
        node.state.tag = 'div'
        node.state.class = 'container' 

        const type = attrs.type
        if (containerStyleRef.hasOwnProperty(type)) {
            const fn = containerStyleRef[type]
            const style = fn(attrs || {})
            if (!style) {
                return 
            }
        ...
        const fn = containerStyleRef[component]
        const style = fn(attrs || {})
        if (!style) {
            return 
        }
        node.state.component = null
        node.state.tag = 'div'
        node.state.class = component
        node.assign('style', style)
        // if (!node.state.style['background-color']) {
            // node.state.style.background = 'blue'
        // }
    }
    // if (node.state.attrs.text) {
        // node.state.value = node.state.attrs.text
    // }
}
