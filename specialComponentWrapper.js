    ...
    lorem: {
        self: {
            handler(args, kwargs) {
                ...
                const [width, height] = args.length == 2 ? args : [args[0], args[0]]
                const value = kwargs?.label ? capitalize(kwargs.label[0]) : `${width} x ${height}`
                return {
                    classes: ['lorem'],  
                    style: {
                        background: kwargs?.fill,
                        "font-weight": kwargs?.label ?  'bold' : null,
                        "font-size": kwargs?.label ?  null : '10pt',
                        width: width + 'pt',
                        height: height + 'pt',
...
const specials = {
    v(node) {
        node.nextSibling.assign('style', 'margin-top', node.state.args[0] + 'pt')
    }
}
