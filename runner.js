...
function runner(line) {
        ...
        if (block.match) {
            ...
            if (match) {
                if (this.options.debug) {
                    alert('matched line: "$1" with block.type:$2', line, block.type)
                }
                // console.log({match})
    ...
    if (block.automaticallyMatch) {
        this.eat()
        this.token.set(block.type, getMatch(this.matches), true)
    ...
    if (this.notDone()) {
        ...
        block.run.call(this, ...args)
        // console.log('aabd', this.matches)
    } else {
        // done
    }
