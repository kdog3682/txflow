...
const LineScannerBlocks = [
    {
        ...
        run() {
            ...
            // the assumption is that it is on a newline by itself
            // if not, the last line will be trimmed off
            // and u may be missing something
            // changed
            // lezer to remove the mbgnl ...
            //
        }
    },
...

    {
        ...
        ignorable: true,
        attributable: false,
        // we do not gather attrs for this item
        match: /^#+/,
        run() {

            throw new Error('skipping py comment')
            const length = this.matches[0].length
            if (length >= 3) {
...

    {
        ...
        googa: true,
        advanceOnMatch: true,
        terminationHook(scanner, token, block) {
            throw 's'
...

    {
        ...
        // isImplicitParent: true,
        match: /^(fooga)/,
        xterminationHook(scanner, indentState) {
            if (
                indentState.until == "newlines" &&
                scanner.token.newlines >= 2
            ) {
                return true
            }
            if (indentState.size) {
                if (indentState.size == 0) {
                    return true
                }
                indentState.size -= 1
            }
...

    {
        ...
        match: /^(com|\$\$\$|flex)/,
        run() {
            throw new Error('skipping block')
            const name = this.matches[0]
            this.token.set("blockName", name, true)
            this.getBlock({ includeEndpoint: false })
        }
...
const blocks = LineScannerBlocks.map(
    editf({ match: ls_matchf, check: ls_testf })
).filter((x) => !x.skip)
