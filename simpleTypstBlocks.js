...
const typstBlocks = [
    ...
    },
    {
        ...
        visit(node) {
            ...
            if (node.attributes) {
                ...
                s += toStringArgumentPretty(node.attributes, {
                    lang: "typst",
                    max_length: 1,
                    noWrapper: true,
                })
    ...
    },
    {
        ...
        visit(node) {
            const computedText = node.contents.filter((x) =>
                x.text
            ).map((x) => fixMathSymbols(x.text)).join(" \\\n")
...
function attrWrap(fnKey, attributes, s) {
    const attrString = toStringArgumentPretty(attributes, {
        lang: "typst",
        max_length: 1000,
        noWrapper: true,
    })
